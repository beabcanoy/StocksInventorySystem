...
const addPurchaseOrder = () => {
    ...
    if (qty < 10) {
        showNotificationModal("Quantity should be 10 or above.");
        return;
    }    
...
function updateTotal() {
    ...
    for (var i = 0; i < rows.length; i++) {
        ...
        if (cells.length >= 4) {
            var amount = parseFloat(cells[4].textContent);
            subtotal += amount;
        }
    }
...
const registerPurchaseOrder = () => {
    ...
    Promise.all([
        ...
        <div class="row">
            ...
            <div class="col">
                ...
                <p id="qtyFeedback" class="text-danger fst-italic" style="display: none;"><small>Quantity should be 10 or above.</small></p>
            </div>
            <div class="col">
    ...
    <tr>
        <th colspan="4">Discount<input type="number" class="ms-2 me-1" id="discountPercentage" style="width:40px !important">%</th>
        <th id="discountValue">0</th>
    </tr>
    <tr>
        <th colspan="4">Tax<input type="number" class="ms-2 me-1" id="taxPercentage" style="width:40px !important">%</th>
        <th id="taxValue">0</th>
    </tr>                              
    <tr>
    ...
    const qtyInput = document.getElementById("rawmat_qty");
        const qtyFeedback = document.getElementById("qtyFeedback");

        // Add event listener to input field to validate quantity
        qtyInput.addEventListener("input", () => {
            const quantity = parseInt(qtyInput.value);
            if (isNaN(quantity) || quantity < 10) {
                qtyFeedback.style.display = "block"; // Show feedback message
            } else {
                qtyFeedback.style.display = "none"; // Hide feedback message
            }
        });
        ...
        const receiveButton = purchaseOrder.status != 1 ?
            ...
            <tr>
                ...
                <td class="text-center">
                    ${receiveButton}
                    <button data-po-id="${purchaseOrder.id}" class="btnView btn border-0 text-info p-1"><i class="fa-solid fa-eye"></i></button>
                </td>
            </tr>
        `;
    });
...
const viewPurchaseOrder = (poId) => {
    ...
    .then(response => {
        ...
        if (purchaseOrder) {
            document.getElementById("receiveModalMainDiv").innerHTML = "";
            document.getElementById("receiveModalMainDiv2").innerHTML = "";
            document.getElementById('receiveModalTitle').innerText = `Purchase Order Details from ${purchaseOrder.purchaseOrder.po_code}`;
            
            const poDetailsHtml = `
                <div class="row">
                    <div class="col">
                        <label for="purchase_order_text" class="form-label mt-2">P.O. Code</label>
                        <input type="text" id="purchase_order_text" class="form-control form-control-sm" value="${purchaseOrder.purchaseOrder.po_code}" disabled>
                    </div>
                    <div class="col">
                        <label for="received_supplier_option" class="form-label mt-2">Supplier</label>
                        <select class="form-select form-select-sm" id="received_supplier_option" aria-label="Default select example" disabled>
                            <option selected value="${purchaseOrder.purchaseOrder.supplier_id}">${purchaseOrder.purchaseOrder.supplier_name}</option>
                        </select>
                    </div>
                </div>
            `;
            ...
            if (purchaseOrder.poItems && purchaseOrder.poItems.length > 0) {
                const poItemsTable = document.createElement('table');
                poItemsTable.classList.add('table');
                const poItemsTableBody = document.createElement('tbody');

                purchaseOrder.poItems.forEach((poItem, index) => {
                    const itemTotalCents = poItem.quantity * poItem.price;
                    const poItemRow = `
                        <tr>
                            <td class="receivedQty">
                                <input type="number" class="form-control form-control-sm" style="width:100px !important" value="${poItem.quantity}" max="${poItem.quantity}" min="0" disabled>
                            </td>
                            <td>${poItem.unit}</td>
                            <td data-rawmat-id="${poItem.rawmat_id}">${poItem.rawmat_name}</td>
                            <td>${poItem.quantity}</td>
                            <td>${poItem.rawmat_netweight}</td>
                            <td>${poItem.price}</td>
                            <td class="amountCell" data-row-index="${index}">${itemTotalCents / 100}</td>
                        </tr>
                    `;
                ...
                const viewSupplierModal = new bootstrap.Modal(document.getElementById('receiveModal'));
                viewSupplierModal.show();
            }
        } else {
            console.error("Error: No purchase order data found");
        }
    })
    .catch(error => {
        console.error("Error fetching purchase order details:", error);
...
const openViewModal = (poId) => {
    ...
    .then(response => {
        ...
        if (purchaseOrder && purchaseOrder.po_code) {
            viewReceivedPO(purchaseOrder, poItems);
        } else {
            console.error('Invalid purchase order data:', purchaseOrder);
        }
    })
...
const viewReceivedPO = (purchaseOrder, poItems) => {
    document.getElementById("receiveModalMainDiv").innerText = "";
    document.getElementById("receiveModalMainDiv2").innerText = "";
    console.log('Purchase Order:', purchaseOrder);
    console.log('PO Items:', poItems);
    ...
    const tableFooter = `
        <tfoot style="text-align:end">
            ...
            <tr>
                <th colspan="4">Discount ${purchaseOrder.discount_percent} %</th>
                <th id="received_discount">${purchaseOrder.discount}</th>
            </tr>
            <tr>
                <th colspan="4">Tax ${purchaseOrder.tax_percent} %</th>
                <th id="received_tax_value">${purchaseOrder.tax}</th>
            </tr>                              
            <tr>
                <th colspan="4">Total</th>
    ...
    // statusLabel.innerText = `Status: ${purchaseOrder.status}`;
    // document.getElementById('receiveModalMainDiv').appendChild(statusLabel);
